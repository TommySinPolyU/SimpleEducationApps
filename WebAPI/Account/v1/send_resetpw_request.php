...
$SQL_FindUser = $conn->prepare("SELECT * FROM $Table_ac WHERE Email = ? AND username = ?");
$SQL_FindUser->execute(array($email, $username));
...
if($count_account != 0){
    ...
    $resetCode = uniqid($regisCode,false);
    $resetLink = "https://sme.dsgshk.com/Account/v1/reset_password.php"."?UID=".$MailerUID."&ResetCode=".$resetCode."&Lang=".$MailerLanguage;
    // Get Last Auto Increment ID.
    $SQL_Get_InquiryID = $conn->prepare(
        "SELECT AUTO_INCREMENT FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA = ? AND TABLE_NAME = ?"
    );
    $SQL_Get_InquiryID->execute(array(DB_NAME,$Table_ResetPWRecords));
    ...
    try {
        ...
        if($MailerLanguage == "zh-hk"){
            $mail->Subject = "SME App 重置密碼申請確定信";
            $mail->Body    = "系統已收到您的重置密碼申請<br>請透過以下的連結進行密碼重置程序：<br>".
            $resetLink."<br><br>請注意：以上連結將會在發出後 30 分鐘失效，<br>如連結已失效，請於應用程式內重新發出重置密碼要求。".
            "<br><br><br><br>---------------------------------------------<br>".
            "此為系統自動發出的電郵，請勿直接回覆此電郵。<br>
            SME App 自動電郵系統<br>".$currenttime;
        } else {
            $mail->Subject = "SME App Password Reset Application Confirmation Letter";
            $mail->Body = "The system received your password reset request<br>Please use the following link to reset your password:<br>".
            $resetLink."<br><br>Please note: the above link will expire 30 minutes after it is issued.<br>If the link has expired, please resend the application for resetting password in Apps.".
            "<br><br><br><br>------------------------------------- --------<br>".
        ...
        $SQL_Insert_InquiryRecord = $conn->prepare(
            "INSERT INTO $Table_ResetPWRecords(UID, resetCode,resetDate) 
            VALUES(?,?,?)"
        );
        $SQL_Insert_InquiryRecord->execute(array($MailerUID, $resetCode,$currenttime));
        $Result = array();
